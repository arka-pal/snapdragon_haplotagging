...
##### written by Arka Pal, last updated 20230712

##### bash usage:  bash ema.sh <samp_no> <samp_list> <fast_dir> <batch> <loc> <species> <ref.fa> <ref.version>
##### SLURM script usage:  srun bash ema.sh $SLURM_ARRAY_TASK_ID $1 $2 $3 $4 $5 $6 $7
##### SLURM job submission:  sbatch --array=1-n job-EMA.slurm <sample_list_filename.txt> <fastq_directory> <sequencing_batch> <sampling_location> <species> <ref.fa> <ref.version>
...
samp_list=$2 #absolute directory path to sample list, .txt file
...
ref_genome=$7
# ref_genome=${WORKDIR}/ref_genome/$ref_gen_ver/Amajus_v3.5.fa #path to reference genome file
ref_gen_ver=$8 #reference genome version used
# ref_gen_ver='v3.5' #reference genome version used ##change manually
...
# done
time parallel --bar -j5 \
   "ema align -t 5 -d -r $ref_genome -p 10x -s {} | \
   samtools sort -@5 -O bam -l 0 -m 4G -o {}.sorted.bam - " ::: ${samp_id}_ema-bin/ema-bin-???

## BWA align for non-BC reads
echo "Running BWA for non-BC file and sort them"
time bwa mem -p -t 40 -M -R $read_group $ref_genome ${samp_id}_ema-bin/ema-nobc | samtools sort -@ 4 -O bam -l 0 -m 4G -o ${samp_id}_ema-bin/ema-nobc.sorted.bam

## Mark duplicates in BWA alignment 
echo "Marking duplicates in BWA alignment"
time sambamba markdup -t 40 -p -l 0 ${samp_id}_ema-bin/ema-nobc.sorted.bam ${samp_id}_ema-bin/ema-nobc-pMarkedup.sorted.bam
...
time sambamba merge -t 40 -p $prefix.sorted.bam ${samp_id}_ema-bin/*.bam

## Change barcodes from base-pairs to AxxBxxCxxDxx
echo "Reconverting 16 base barcodes to haplotag barcodes"
time samtools view -h $prefix.sorted.bam |\
